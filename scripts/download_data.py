...
class TushareDataDownloader:
    ...
    # 股指期货合约映射（Tushare 使用 .CFFEX 后缀）
    FUTURES_MAP = {
        "IF": "IF.CFFEX",  # 沪深300股指期货
    ...
    # 防爬策略：请求间隔（秒）
    # Tushare 免费账号限制：每分钟最多调用 fut_basic 2次
    REQUEST_DELAY = 35  # 35秒延迟，确保不超过每分钟2次的限制
    MAX_RETRIES = 3
...
def main():
    ...
    start_date = datetime(2025, 3, 13)  # 测试：先下载1个月
